...
export class CategoryController {
    ...
    @Get('allcourse/:name')
    @ApiOperation({
        "summary" : "카테고리와 관련된 모든 강의 조회하는 요청",
        "description" : "모든 강의를 배열 형태로 반환한다."
    })
    async getAllCourseWithCategory(@Param("name") categoryName : string) : Promise<CourseCategory[]>{
        return await this.categoryService.getAllCourseWithCategory(categoryName);
    }
    ...
    async getOneAnswer(@Param("name") categoryName : string) : Promise<Category>{
    ...
    createAnswer(@Body() createCategoryDto : CreateCategoryDto){
    ...
    patchAnswer(@Param('name') categoryName : string,@Body() updateCategoryDto : UpdateCategoryDto){
